...
class _CookProfileEditScreenState extends State<CookProfileEditScreen> {
                      ...
                      children: [
                        ...
                        _buildMenuCard([
                          ...
                          _ProfileMenuItem(
                            icon: Icons.store_mall_directory_outlined,
                            title: 'Pickup Locations',
                            subtitle: '${cook?.pickupLocations.length ?? 0} active locations',
                            onTap: () => _showPickupLocationsSheet(context),
                          ),
                        ]),
  ...
  void _showPickupLocationsSheet(BuildContext context) {
    showModalBottomSheet(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.white,
      ...
      builder: (ctx) => DraggableScrollableSheet(
        ...
        minChildSize: 0.4,
        expand: false,
        builder: (_, scrollCtrl) => Consumer<AuthProvider>(
          builder: (_, auth, __) {
            final cook = auth.currentUser?.cookProfile;
            return Padding(
              padding: const EdgeInsets.fromLTRB(24, 24, 24, 0),
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  _buildSheetHeader('Pickup Locations'),
                  Row(
                    ...
                    children: [
                      ...
                      IconButton(
                        onPressed: () => _showAddPickupLocationDialog(ctx),
                  ...
                  const SizedBox(height: 12),
                  Expanded(
                    child: (cook == null || cook.pickupLocations.isEmpty)
                        ? Center(
                            child: Column(
                        ...
                        : ListView.separated(
                            ...
                            itemCount: cook.pickupLocations.length,
                            separatorBuilder: (_, __) => const Divider(height: 1),
                            itemBuilder: (_, i) {
                              final loc = cook.pickupLocations[i];
                              return ListTile(
                                leading: CircleAvatar(
                                  backgroundColor: AppTheme.primaryOrange.withValues(alpha: 0.1),
                                  child: const Icon(Icons.location_on, color: AppTheme.primaryOrange, size: 20),
                                ),
                                title: Text(loc.label, style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 14)),
                                subtitle: Text(
                                  loc.fullAddress,
                                ...
                                trailing: IconButton(
                                  ...
                                  onPressed: () async {
                                    try {
                                      await auth.deletePickupLocation(loc.id);
  ...
  Future<void> _showAddPickupLocationDialog(BuildContext contextContext) async {
    final labelCtrl = TextEditingController();
    final streetCtrl = TextEditingController();
    final cityCtrl = TextEditingController();
    final stateCtrl = TextEditingController();
    ...
    // Opening this dialog above the bottom sheet
    final result = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              TextField(
                controller: labelCtrl,
                decoration: const InputDecoration(labelText: 'Label', hintText: 'e.g., Home, Kitchen...'),
              ),
              const SizedBox(height: 8),
              TextField(
                controller: streetCtrl,
    ...
    if (result == true && mounted) {
      if (labelCtrl.text.isEmpty || streetCtrl.text.isEmpty || cityCtrl.text.isEmpty) {
        _showError('Label, street and suburb are required');
        return;
      }
      ...
      try {
        ...
        _showSuccess('Pickup location added!');
      } catch (e) {
